USE [GD1C2022]
GO
--SELECT * FROM gd_esquema.Maestra
IF NOT EXISTS (SELECT * FROM sys.schemas where name = 'SUB_SELECT')
BEGIN 
	EXEC ('CREATE SCHEMA [SUB_SELECT]')
END
GO

CREATE TABLE [SUB_SELECT].[Piloto]
	(
	[ID_PILOTO] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Piloto PRIMARY KEY,--Agergo el ID para idenficarla
	[PILOTO_NOMBRE] [nvarchar](50) NOT NULL,
	[PILOTO_APELLIDO] [nvarchar](50) NOT NULL,
	[PILOTO_NRO_DOCUMENTO] [nvarchar](15) NULL,
	[PILOTO_TIPO_DOCUMENTO] [nvarchar](5) NULL,
	[PILOTO_NACIONALIDAD] [nvarchar] (50) NOT NULL,
	[PILOTO_FECHA_NACIMIENTO] [date] NOT NULL,
	--UNIQUE (PILOTO_NRO_DOCUMENTO, PILOTO_TIPO_DOCUMENTO)
	)
GO	

CREATE TABLE [SUB_SELECT].[Escuderia]
	(
	[ID_ESCUDERIA] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Escuderia PRIMARY KEY,--Agergo el ID para idenficarla
	[ESCUDERIA_NOMBRE] [nvarchar](255) NOT NULL,
	[ESCUDERIA_NACIONALIDAD] [nvarchar] (255) NOT NULL
	)
GO	

CREATE TABLE [SUB_SELECT].[Auto]
	(
	[ID_AUTO] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Auto PRIMARY KEY,--Agergo el ID para idenficarla
	[AUTO_NUMERO] int NOT NULL,
	[ID_ESCUDERIA] int NOT NULL,							--FK a Escuderia o lo relaciono con el ID de la escuderia
	[AUTO_MODELO] [nvarchar](255) NOT NULL,
	[AUTO_DESCRIPCION] [nvarchar] (255) NULL,
	UNIQUE(AUTO_NUMERO,AUTO_DESCRIPCION)
	)
GO		

CREATE TABLE [SUB_SELECT].[Medicion]
	(
	[ID_MEDICION] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Medicion PRIMARY KEY,--Agergo el ID para idenficarla --FK
	[FECHA_HORA] [datetime] NULL,
	[ID_CARRERA] int NOT NULL,							    --FK a Carrera o lo relaciono con el ID de la carrera
	[ID_SECTOR] int NOT NULL,							    --FK a Sector o lo relaciono con el ID deL sector
	[DISTANCIA_RECORRIDA_CARRERA] [decimal] (18,6) NOT NULL,
	[ID_AUTO] int NOT NULL                             --FK a Auto o lo relaciono con el ID del auto
	)
GO	

CREATE TABLE [SUB_SELECT].[Medicion_Auto]
	(
	[ID_MEDICION] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Medicion_Auto PRIMARY KEY,--Agergo el ID para idenficarla
	[ID_AUTO] int NOT NULL,                             --FK a Auto o lo relaciono con el ID del auto
	[ID_PILOTO] int NOT NULL,							    --FK a Piloto o lo relaciono con el ID del piloto
	[VUELTA_NUMERO] [decimal](18,0) NOT NULL,
	[DISTANCIA_RECORRIDA_VUELTA] [decimal] (18,2) NOT NULL,
	[TIEMPO_DE_VUELTA] [decimal] (18,10) NOT NULL,
	[POSICION] [decimal] (18,0) NOT NULL,
	[VELOCIDAD] [decimal] (18,2) NOT NULL,
	[COMBUSTIBLE] [decimal] (18,2) NOT NULL
	)
GO

CREATE TABLE [SUB_SELECT].[Medicion_Motor]
	(
	[ID_MEDICION] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Medicion_Motor PRIMARY KEY,--Agergo el ID para idenficarla
	[ID_MOTOR] int NOT NULL,                             --FK a Motor o lo relaciono con el ID del motor
	[POTENCIA_MOMENTANEA] [decimal](18,6) NOT NULL,
	[TEMP_ACEITE] [decimal] (18,6) NOT NULL,
	[TEMP_AGUA] [decimal] (18,6) NOT NULL,
	[RPM] [decimal] (18,6) NOT NULL
	)
GO	

CREATE TABLE [SUB_SELECT].[Medicion_Neumatico]
	(
	[ID_MEDICION] int NOT NULL,--Agergo el ID para idenficarla
	[PROFUNDIDAD] [decimal] (18,6) NOT NULL,
	[POSICION] [nvarchar](255) NOT NULL,                     --PK
	[PRESION] [decimal] (18,6) NOT NULL,
	[TEMPERATURA] [decimal] (18,6) NOT NULL,
	[ID_NEUMATICO] int NOT NULL,							    --FK a Neumatico o lo relaciono con el ID del neumatico
	PRIMARY KEY(ID_MEDICION, POSICION)
	)
GO	

CREATE TABLE [SUB_SELECT].[Medicion_Frenos]
	(
	[ID_MEDICION] int NOT NULL,--Agergo el ID para idenficarla
	[GROSOR_PASTILLA] [nvarchar] NOT NULL,
	[POSICION] [nvarchar](255) NOT NULL,                     --PK
	[TEMPERATURA] [decimal] (18,2) NOT NULL,
	[ID_FRENO] int NOT NULL,							    --FK a Freno o lo relaciono con el ID del freno
	PRIMARY KEY(ID_MEDICION, POSICION)
	)
GO	

CREATE TABLE [SUB_SELECT].[Medicion_Caja_De_Cambios]
	(
	[ID_MEDICION] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Medicion_Caja PRIMARY KEY,--Agergo el ID para idenficarla
	[CAJA] [nvarchar] (255) NOT NULL,
	[TEMPERATURA_ACEITE] [decimal] (18,2) NOT NULL,
	[RPM] [decimal] (18,2) NOT NULL,
	[DESGASTE_PORCENTUAL_ACUMULADO] [decimal] (18,2) NOT NULL,
	[ID_CAJA_CAMBIO] int NOT NULL,							    --FK a Caja de cambios o lo relaciono con el ID de la caja de cambios
	)
GO	

CREATE TABLE [SUB_SELECT].[Motor]
	(
	[ID_MOTOR] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Motor PRIMARY KEY,--Agergo el ID para idenficarla
	[MODELO] [nvarchar](255) NOT NULL,
	[NRO_SERIE] [nvarchar] (255) NOT NULL
	)
GO	

CREATE TABLE [SUB_SELECT].[Neumatico]
	(
	[ID_NEUMATICO] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Neumatico PRIMARY KEY,--Agergo el ID para idenficarla
	[NEUMATICO_NOMBRE] [nvarchar](255) NULL,
	[NEUMATICO_TIPO] [nvarchar] (255) NULL,
	[NRO_SERIE] [nvarchar] (255) NOT NULL
	)
GO	

CREATE TABLE [SUB_SELECT].[Freno]
	(
	[ID_FRENO] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Freno PRIMARY KEY,--Agergo el ID para idenficarla
	[FRENO_NOMBRE] [nvarchar](255) NULL,
	[NRO_SERIE] [nvarchar] (255) NOT NULL,
	[TAMANIO_DISCO] [decimal] (18,2) NOT NULL
	)
GO	

CREATE TABLE [SUB_SELECT].[Caja_de_Cambios]
	(
	[ID_CAJA_CAMBIO] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Caja_Cambio PRIMARY KEY,--Agergo el ID para idenficarla
	[CAJA_CAMBIO_NOMBRE] [nvarchar](255) NULL,
	[CAJA_CAMBIO_MODELO] [nvarchar] (50) NOT NULL,
	[NRO_SERIE] [nvarchar] (255) NOT NULL
	)
GO	

CREATE TABLE [SUB_SELECT].[Carrera]
	(
	[ID_CARRERA] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Carrera PRIMARY KEY,--Agergo el ID para idenficarla
	[ID_CIRCUITO] int NOT NULL,                             --FK a circuito o lo relaciono con el ID del circuito
	[CARRERA_FECHA_INICIO] [DATE] NOT NULL,
	[CARRERA_FECHA_FIN] [DATE] NULL,
	[VUELTAS_TOTAL] int NOT NULL,
	[CARRERA_CLIMA] [nvarchar] (100) NOT NULL,
	[CARRERA_LONGITUD] [decimal] (18,2) NOT NULL
	)
GO

CREATE TABLE [SUB_SELECT].[Circuito]
	(
	[ID_CIRCUITO] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Circuito PRIMARY KEY,--Agergo el ID para idenficarla
	[CIRCUITO_NOMBRE] [nvarchar](255) NOT NULL,
	[CIRCUITO_DESCRIPCION] [nvarchar] (50) NULL,
	[CIRCUITO_PAIS] [nvarchar] (255) NOT NULL
	)
GO	

CREATE TABLE [SUB_SELECT].[Sector]
	(
	[ID_SECTOR] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Sector PRIMARY KEY,--Agergo el ID para idenficarla
	[ID_CIRCUITO] int NOT NULL,                             --FK a circuito o lo relaciono con el ID del circuito
	[SECTOR_NOMBRE] [nvarchar] (255) NULL,
	[SECTOR_DESCRIPCION] [nvarchar] (255) NULL,
	[SECTOR_LONGITUD] [decimal] (18,2) NULL,
	[SECTOR_TIPO] [nvarchar] (255) NOT NULL
	)
GO

CREATE TABLE [SUB_SELECT].[Incidente]
	(
	[ID_INCIDENTE] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Incidente PRIMARY KEY,--Agergo el ID para idenficarla
	[INCIDENTE_TIEMPO] [TIMESTAMP] NULL,
	[INCIDENTE_BANDERA] [nvarchar] (255) NULL,
	[ID_SECTOR] int NOT NULL                             --FK a Sector o lo relaciono con el ID del Sector
	)
GO

CREATE TABLE [SUB_SELECT].[Auto_Incidentado]
	(
	[ID_AUTO_INCIDENTADO] int NOT NULL,
	[ID_INCIDENTE] int NOT NULL,  --PK,FK
	[ID_AUTO] int NOT NULL,  --PK, FK
	[ID_TIPO_INCIDENTE] int NOT NULL, --FK a Tipo Incidente
	[NRO_VUELTA] [decimal] (18,0) NULL,
	PRIMARY KEY(ID_AUTO_INCIDENTADO, ID_INCIDENTE, ID_AUTO)
	)
GO

CREATE TABLE [SUB_SELECT].[Tipo_Incidente]
	(
	[ID_TIPO_INCIDENTE] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Tipo_Incidente PRIMARY KEY,--Agergo el ID para idenficarla
	[TIPO_INCIDENTE] [nvarchar] (255) NULL,
	[TIPO_INCIDENTE_DESCRIPCION] [nvarchar] (255) NULL                          
	)
GO

CREATE TABLE [SUB_SELECT].[Parada_En_Box]
	(
	[ID_PARADA] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Parada_Box PRIMARY KEY,--Agergo el ID para idenficarla
	[ID_AUTO] int NOT NULL,                             --FK a Auto o lo relaciono con el ID del auto
	[NUMERO_VUELTA] [decimal] (18,0) NULL,
	[PARADA_DURACION] [decimal] (18,2) NULL
	)
GO

CREATE TABLE [SUB_SELECT].[Cambio_De_Neumatico]
	(
	[ID_CAMBIO_NEUMATICO] int NOT NULL IDENTITY (1, 1) CONSTRAINT PK_Cambio_Neumatico PRIMARY KEY,--Agergo el ID para idenficarla
	[ID_NEUMATICO_ANTERIOR] int NOT NULL,                             --FK a Neumatico o lo relaciono con el ID del Neumatico anterior
	[ID_NEUMATICO_NUEVO] int NOT NULL,                             --FK a Neumatico o lo relaciono con el ID del Neumatico nuevo
	[ID_PARADA] int NOT NULL,                             --FK a Parada o lo relaciono con el ID de la Parada
	[POSICION] [nvarchar] (255) NULL
	)
GO

--Se agregan las FK

--Auto
ALTER TABLE [SUB_SELECT].[Auto]
ADD CONSTRAINT FK_ESCUDERIA FOREIGN KEY(ID_ESCUDERIA) REFERENCES [SUB_SELECT].Escuderia(ID_ESCUDERIA);

--Medicion
--ALTER TABLE [SUB_SELECT].[Medicion]
--ADD CONSTRAINT FK_MEDICION FOREIGN KEY(ID_MEDICION) REFERENCES [SUB_SELECT].Medicion_Auto(ID_MEDICION);--, [SUB_SELECT].Medicion_Motor(ID_MEDICION), [SUB_SELECT].Medicion_Neumatico(ID_MEDICION), [SUB_SELECT].Medicion_Frenos(ID_MEDICION), [SUB_SELECT].Medicion_Caja_De_Cambios(ID_MEDICION);

ALTER TABLE [SUB_SELECT].[Medicion]
ADD CONSTRAINT FK_CARRERA FOREIGN KEY(ID_CARRERA) REFERENCES [SUB_SELECT].Carrera(ID_CARRERA);

ALTER TABLE [SUB_SELECT].[Medicion]
ADD CONSTRAINT FK_SECTOR_MEDICION FOREIGN KEY(ID_SECTOR) REFERENCES [SUB_SELECT].Sector(ID_SECTOR);

ALTER TABLE [SUB_SELECT].[Medicion]
ADD CONSTRAINT FK_AUTO_MEDICION FOREIGN KEY(ID_AUTO) REFERENCES [SUB_SELECT].Auto(ID_AUTO);

--Medicion Auto
ALTER TABLE [SUB_SELECT].[Medicion_Auto]
ADD CONSTRAINT FK_NRO_AUTO FOREIGN KEY(ID_AUTO) REFERENCES [SUB_SELECT].Auto(ID_AUTO);

ALTER TABLE [SUB_SELECT].[Medicion_Auto]
ADD CONSTRAINT FK_PILOTO FOREIGN KEY(ID_PILOTO) REFERENCES [SUB_SELECT].Piloto(ID_PILOTO);

--Medicion Motor
ALTER TABLE [SUB_SELECT].[Medicion_Motor]
ADD CONSTRAINT FK_MOTOR FOREIGN KEY(ID_MOTOR) REFERENCES [SUB_SELECT].Motor(ID_MOTOR);

--Medicion Neumatico
ALTER TABLE [SUB_SELECT].[Medicion_Neumatico]
ADD CONSTRAINT FK_NEUMATICO FOREIGN KEY(ID_NEUMATICO) REFERENCES [SUB_SELECT].Neumatico(ID_NEUMATICO);

--Medicion Freno
ALTER TABLE [SUB_SELECT].[Medicion_Frenos]
ADD CONSTRAINT FK_FRENO FOREIGN KEY(ID_FRENO) REFERENCES [SUB_SELECT].Freno(ID_FRENO);

--Medicion Caja De Cambios
ALTER TABLE [SUB_SELECT].[Medicion_Caja_De_Cambios]
ADD CONSTRAINT FK_CAJA_CAMBIO FOREIGN KEY(ID_CAJA_CAMBIO) REFERENCES [SUB_SELECT].Caja_De_Cambios(ID_CAJA_CAMBIO);

--Carrera
ALTER TABLE [SUB_SELECT].[Carrera]
ADD CONSTRAINT FK_CIRCUITO_CARRERA FOREIGN KEY(ID_CIRCUITO) REFERENCES [SUB_SELECT].Circuito(ID_CIRCUITO);

--Sector
ALTER TABLE [SUB_SELECT].[Sector]
ADD CONSTRAINT FK_CIRCUITO_SECTOR FOREIGN KEY(ID_CIRCUITO) REFERENCES [SUB_SELECT].Circuito(ID_CIRCUITO);

--Incidente
ALTER TABLE [SUB_SELECT].[Incidente]
ADD CONSTRAINT FK_SECTOR_INCIDENTE FOREIGN KEY(ID_SECTOR) REFERENCES [SUB_SELECT].Sector(ID_SECTOR);

--Auto Incidentado
ALTER TABLE [SUB_SELECT].[Auto_Incidentado]
ADD CONSTRAINT FK_INCIDENTE FOREIGN KEY(ID_INCIDENTE) REFERENCES [SUB_SELECT].Incidente(ID_INCIDENTE);

ALTER TABLE [SUB_SELECT].[Auto_Incidentado]
ADD CONSTRAINT FK_NUMERO_AUTO FOREIGN KEY(ID_AUTO) REFERENCES [SUB_SELECT].Auto(ID_AUTO);

ALTER TABLE [SUB_SELECT].[Auto_Incidentado]
ADD CONSTRAINT FK_TIPO_INCIDENTE FOREIGN KEY(ID_TIPO_INCIDENTE) REFERENCES [SUB_SELECT].Tipo_Incidente(ID_TIPO_INCIDENTE);

--Parada en Box
ALTER TABLE [SUB_SELECT].[Parada_En_Box]
ADD CONSTRAINT FK_AUTO_PARADA FOREIGN KEY(ID_AUTO) REFERENCES [SUB_SELECT].Auto(ID_AUTO);

--Cambio De Neumaticos
ALTER TABLE [SUB_SELECT].[Cambio_De_Neumatico]
ADD CONSTRAINT FK_NEUMATICO_ANTERIOR FOREIGN KEY(ID_NEUMATICO_ANTERIOR) REFERENCES [SUB_SELECT].Neumatico(ID_NEUMATICO);

ALTER TABLE [SUB_SELECT].[Cambio_De_Neumatico]
ADD CONSTRAINT FK_NEUMATICO_NUEVO FOREIGN KEY(ID_NEUMATICO_NUEVO) REFERENCES [SUB_SELECT].Neumatico(ID_NEUMATICO);

ALTER TABLE [SUB_SELECT].[Cambio_De_Neumatico]
ADD CONSTRAINT FK_PARADA FOREIGN KEY(ID_PARADA) REFERENCES [SUB_SELECT].Parada_En_Box(ID_PARADA);

----------------------------------------------	MIGRACIÓN  ----------------------------------------------

INSERT INTO [SUB_SELECT].Piloto
(PILOTO_NOMBRE, PILOTO_APELLIDO, PILOTO_NACIONALIDAD, PILOTO_FECHA_NACIMIENTO)
SELECT PILOTO_NOMBRE, PILOTO_APELLIDO, PILOTO_NACIONALIDAD, PILOTO_FECHA_NACIMIENTO FROM gd_esquema.Maestra;--ok

INSERT INTO [SUB_SELECT].Escuderia
(ESCUDERIA_NOMBRE, ESCUDERIA_NACIONALIDAD)
SELECT ESCUDERIA_NOMBRE, ESCUDERIA_NACIONALIDAD FROM gd_esquema.Maestra--ok

INSERT INTO [SUB_SELECT].Auto
(AUTO_NUMERO, e.ID_ESCUDERIA, AUTO_MODELO)
SELECT AUTO_NUMERO, AUTO_MODELO FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Escuderia e ON e.ESCUDERIA_NOMBRE = gd_esquema.Maestra.ESCUDERIA_NOMBRE ----TODO ¿Está bien?

INSERT INTO [SUB_SELECT].Carrera
(CARRERA_FECHA_INICIO, ID_CARRERA, CARRERA_CLIMA, CARRERA_LONGITUD, VUELTAS_TOTAL, ID_CIRCUITO)
SELECT CARRERA_FECHA, CODIGO_CARRERA, CARRERA_CLIMA, CARRERA_TOTAL_CARRERA, CARRERA_CANT_VUELTAS, CIRCUITO_CODIGO FROM gd_esquema.Maestra

INSERT INTO [SUB_SELECT].Circuito
(ID_CIRCUITO, CIRCUITO_NOMBRE, CIRCUITO_PAIS)
SELECT CIRCUITO_CODIGO, CIRCUITO_NOMBRE, CIRCUITO_PAIS FROM gd_esquema.Maestra

INSERT INTO [SUB_SELECT].Sector
(ID_SECTOR, SECTOR_TIPO, SECTOR_LONGITUD, ID_CIRCUITO)
SELECT CODIGO_SECTOR, SECTO_TIPO, SECTOR_DISTANCIA, CIRCUITO_CODIGO FROM gd_esquema.Maestra

INSERT INTO [SUB_SELECT].Incidente
(INCIDENTE_BANDERA, ID_SECTOR, INCIDENTE_TIEMPO)
SELECT INCIDENTE_BANDERA, CODIGO_SECTOR, TELE_AUTO_TIEMPO_VUELTA FROM gd_esquema.Maestra

INSERT INTO [SUB_SELECT].Tipo_incidente
(TIPO_INCIDENTE)
SELECT INCIDENTE_TIPO  FROM gd_esquema.Maestra

--------------------------------------AUTO INCIDENTADO-------------------------------------	TODO
INSERT INTO [SUB_SELECT].Auto_incidentado 
(NRO_VUELTA, i.ID_INCIDENTE, a.ID_AUTO, t.ID_TIPO_INCIDENTE)
SELECT INCIDENTE_NUMERO_VUELTA FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Incidente i ON i.INCIDENTE_BANDERA = Maestra.INCIDENTE_BANDERA AND i.INCIDENTE_TIEMPO = Maestra.INCIDENTE_NUMERO_VUELTA
JOIN [SUB_SELECT].Auto a ON a.AUTO_MODELO = Maestra.AUTO_MODELO and A.AUTO_NUMERO = Maestra.AUTO_NUMERO
JOIN [SUB_SELECT].Tipo_incidente t ON t.TIPO_INCIDENTE = Maestra.INCIDENTE_TIPO

INSERT INTO [SUB_SELECT].Parada_En_box
(NUMERO_VUELTA, PARADA_DURACION, ID_AUTO)
SELECT PARADA_BOX_TIEMPO, PARADA_BOX_VUELTA, AUTO_NUMERO FROM gd_esquema.Maestra

INSERT INTO [SUB_SELECT].Medicion_auto
(ID_MEDICION, VUELTA_NUMERO, DISTANCIA_RECORRIDA_VUELTA, POSICION, TIEMPO_DE_VUELTA, VELOCIDAD, COMBUSTIBLE, p.ID_PILOTO, a.ID_AUTO)
SELECT TELE_AUTO_CODIGO, TELE_AUTO_NUMERO_VUELTA, TELE_AUTO_DISTANCIA_VUELTA, TELE_AUTO_POSICION, TELE_AUTO_TIEMPO_VUELTA,
		TELE_AUTO_VELOCIDAD, TELE_AUTO_COMBUSTIBLE FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Piloto p ON p.PILOTO_NOMBRE = Maestra.PILOTO_NOMBRE
JOIN [SUB_SELECT].Auto a ON a.AUTO_MODELO = Maestra.AUTO_MODELO AND a.AUTO_NUMERO = Maestra.AUTO_NUMERO

INSERT INTO [SUB_SELECT].Caja_de_cambios
(CAJA_CAMBIO_MODELO, NRO_SERIE)
SELECT TELE_CAJA_MODELO, TELE_CAJA_NRO_SERIE FROM gd_esquema.Maestra

INSERT INTO [SUB_SELECT].Medicion_caja_de_cambios
(RPM, TEMPERATURA_ACEITE, DESGASTE_PORCENTUAL_ACUMULADO, c.ID_CAJA_CAMBIO)
SELECT TELE_CAJA_RPM, TELE_CAJA_TEMP_ACEITE, TELE_CAJA_DESGASTE FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Caja_de_Cambios c ON c.CAJA_CAMBIO_MODELO = Maestra.TELE_CAJA_MODELO 
						AND c.NRO_SERIE = Maestra.TELE_CAJA_NRO_SERIE 							

INSERT INTO [SUB_SELECT].Motor
(MODELO, NRO_SERIE)
SELECT TELE_MOTOR_MODELO, TELE_MOTOR_NRO_SERIE FROM gd_esquema.Maestra

INSERT INTO [SUB_SELECT].Medicion_Motor
(POTENCIA_MOMENTANEA, RPM, TEMP_ACEITE, TEMP_AGUA, m.ID_MOTOR)
SELECT TELE_MOTOR_POTENCIA, TELE_MOTOR_RPM, TELE_MOTOR_TEMP_ACEITE, TELE_MOTOR_TEMP_AGUA FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Motor m ON m.MODELO = Maestra.TELE_MOTOR_MODELO
			AND m.NRO_SERIE = Maestra.TELE_MOTOR_NRO_SERIE

INSERT INTO [SUB_SELECT].Neumatico
(NRO_SERIE, n.NEUMATICO_TIPO)
SELECT TELE_NEUMATICO1_NRO_SERIE FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Neumatico n ON n.NRO_SERIE = Maestra.NEUMATICO1_NRO_SERIE_NUEVO
INSERT INTO [SUB_SELECT].Neumatico
(NRO_SERIE, n.NEUMATICO_TIPO)
SELECT TELE_NEUMATICO2_NRO_SERIE FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Neumatico n ON n.NRO_SERIE = Maestra.NEUMATICO2_NRO_SERIE_NUEVO
INSERT INTO [SUB_SELECT].Neumatico
(NRO_SERIE, n.NEUMATICO_TIPO)
SELECT TELE_NEUMATICO3_NRO_SERIE FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Neumatico n ON n.NRO_SERIE = Maestra.NEUMATICO3_NRO_SERIE_NUEVO
INSERT INTO [SUB_SELECT].Neumatico
(NRO_SERIE, n.NEUMATICO_TIPO)
SELECT TELE_NEUMATICO4_NRO_SERIE FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Neumatico n ON n.NRO_SERIE = Maestra.NEUMATICO4_NRO_SERIE_NUEVO

INSERT INTO [SUB_SELECT].Medicion_Neumatico
(POSICION, PRESION, PROFUNDIDAD, TEMPERATURA, n.ID_NEUMATICO)
SELECT TELE_NEUMATICO1_POSICION, TELE_NEUMATICO1_PRESION, TELE_NEUMATICO1_PROFUNDIDAD, TELE_NEUMATICO1_TEMPERATURA FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Neumatico n ON n.NRO_SERIE = Maestra.TELE_NEUMATICO1_NRO_SERIE
INSERT INTO [SUB_SELECT].Medicion_Neumatico
(POSICION, PRESION, PROFUNDIDAD, TEMPERATURA, n.ID_NEUMATICO)
SELECT TELE_NEUMATICO2_POSICION, TELE_NEUMATICO2_PRESION, TELE_NEUMATICO2_PROFUNDIDAD, TELE_NEUMATICO2_TEMPERATURA FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Neumatico n ON n.NRO_SERIE = Maestra.TELE_NEUMATICO2_NRO_SERIE
INSERT INTO [SUB_SELECT].Medicion_Neumatico
(POSICION, PRESION, PROFUNDIDAD, TEMPERATURA, n.ID_NEUMATICO)
SELECT TELE_NEUMATICO3_POSICION, TELE_NEUMATICO3_PRESION, TELE_NEUMATICO3_PROFUNDIDAD, TELE_NEUMATICO3_TEMPERATURA FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Neumatico n ON n.NRO_SERIE = Maestra.TELE_NEUMATICO3_NRO_SERIE
INSERT INTO [SUB_SELECT].Medicion_Neumatico
(POSICION, PRESION, PROFUNDIDAD, TEMPERATURA, n.ID_NEUMATICO)
SELECT TELE_NEUMATICO4_POSICION, TELE_NEUMATICO4_PRESION, TELE_NEUMATICO4_PROFUNDIDAD, TELE_NEUMATICO4_TEMPERATURA FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Neumatico n ON n.NRO_SERIE = Maestra.TELE_NEUMATICO4_NRO_SERIE

INSERT INTO [SUB_SELECT].Freno
(NRO_SERIE, TAMANIO_DISCO)
SELECT TELE_FRENO1_NRO_SERIE, TELE_FRENO1_TAMANIO_DISCO FROM gd_esquema.Maestra
INSERT INTO [SUB_SELECT].Freno
(NRO_SERIE, TAMANIO_DISCO)
SELECT TELE_FRENO2_NRO_SERIE, TELE_FRENO2_TAMANIO_DISCO FROM gd_esquema.Maestra
INSERT INTO [SUB_SELECT].Freno
(NRO_SERIE, TAMANIO_DISCO)
SELECT TELE_FRENO3_NRO_SERIE, TELE_FRENO3_TAMANIO_DISCO FROM gd_esquema.Maestra
INSERT INTO [SUB_SELECT].Freno
(NRO_SERIE, TAMANIO_DISCO)
SELECT TELE_FRENO4_NRO_SERIE, TELE_FRENO4_TAMANIO_DISCO FROM gd_esquema.Maestra

INSERT INTO [SUB_SELECT].Medicion_frenos
(POSICION, TEMPERATURA, f.ID_FRENO)
SELECT TELE_FRENO1_POSICION, TELE_FRENO1_TEMPERATURA FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Freno f ON f.NRO_SERIE = Maestra.TELE_FRENO1_NRO_SERIE
			AND f.TAMANIO_DISCO = Maestra.TELE_FRENO1_TAMANIO_DISCO
INSERT INTO [SUB_SELECT].Medicion_frenos
(POSICION, TEMPERATURA, f.ID_FRENO)
SELECT TELE_FRENO2_POSICION, TELE_FRENO2_TEMPERATURA FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Freno f ON f.NRO_SERIE = Maestra.TELE_FRENO2_NRO_SERIE
			AND f.TAMANIO_DISCO = Maestra.TELE_FRENO2_TAMANIO_DISCO
INSERT INTO [SUB_SELECT].Medicion_frenos
(POSICION, TEMPERATURA, f.ID_FRENO)
SELECT TELE_FRENO3_POSICION, TELE_FRENO3_TEMPERATURA FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Freno f ON f.NRO_SERIE = Maestra.TELE_FRENO3_NRO_SERIE
			AND f.TAMANIO_DISCO = Maestra.TELE_FRENO3_TAMANIO_DISCO
INSERT INTO [SUB_SELECT].Medicion_frenos
(POSICION, TEMPERATURA, f.ID_FRENO)
SELECT TELE_FRENO4_POSICION, TELE_FRENO4_TEMPERATURA FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Freno f ON f.NRO_SERIE = Maestra.TELE_FRENO4_NRO_SERIE
			AND f.TAMANIO_DISCO = Maestra.TELE_FRENO4_TAMANIO_DISCO

INSERT INTO [SUB_SELECT].Cambio_De_Neumatico
(POSICION, p.ID_PARADA, n2.ID_NEUMATICO_ANTERIOR, n.ID_NEUMATICO_NUEVO)
SELECT TELE_NEUMATICO1_POSICION FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Neumatico n ON n.NRO_SERIE = Maestra.NEUMATICO1_NRO_SERIE_NUEVO
				AND n.NEUMATICO_TIPO = Maestra.NEUMATICO1_TIPO_NUEVO
JOIN [SUB_SELECT].Neumatico n2 ON n.NRO_SERIE = Maestra.NEUMATICO1_NRO_SERIE_VIEJO
				  AND n.NEUMATICO_TIPO = Maestra.NEUMATICO1_TIPO_VIEJO
JOIN [SUB_SELECT].Auto a ON a.AUTO_MODELO = Maestra.AUTO_MODELO AND a.AUTO_NUMERO = Maestra.AUTO_NUMERO
JOIN [SUB_SELECT].Parada_En_Box p ON p.PARADA_DURACION = Maestra.PARADA_BOX_TIEMPO
					AND p.NUMERO_VUELTA = Maestra.PARADA_BOX_VUELTA
					AND p.ID_AUTO = a.id_auto

INSERT INTO [SUB_SELECT].Medicion
(c.ID_CARRERA, ID_SECTOR, DISTANCIA_RECORRIDA_CARRERA, a.ID_AUTO)
SELECT CODIGO_SECTOR, TELE_AUTO_DISTANCIA_CARRERA FROM gd_esquema.Maestra
JOIN [SUB_SELECT].Carrera c ON c.CARRERA_FECHA_INICIO = Maestra.CARRERA_FECHA AND c.CARRERA_LONGITUD = Maestra.CARRERA_TOTAL_CARRERA
				AND c.VUELTAS_TOTAL = Maestra.CARRERA_CANT_VUELTAS AND c.CARRERA_CLIMA = Maestra.CARRERA_CLIMA
JOIN [SUB_SELECT].Auto a ON a.AUTO_MODELO = Maestra.AUTO_MODELO AND a.AUTO_NUMERO = Maestra.AUTO_NUMERO